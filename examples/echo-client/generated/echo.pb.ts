...
export interface Echo_EchoRequest {
  message: string;
}

export interface Echo_EchoResponse {
  reply: string;
  timestamp: bigint;
}

export const Echo_EchoRequest = {
  encode(message: Echo_EchoRequest): Buffer {
    const parts: Buffer[] = [];
  ...
  decode(buffer: Buffer): Echo_EchoRequest {
    let offset = 0;
    let message = '';
...
export const Echo_EchoResponse = {
  encode(message: Echo_EchoResponse): Buffer {
    const parts: Buffer[] = [];
  ...
  decode(buffer: Buffer): Echo_EchoResponse {
    let offset = 0;
    let reply = '';
    let timestamp = 0n;
